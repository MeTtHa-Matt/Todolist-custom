...
$count = $pdo->query("SELECT COUNT(id) FROM todo WHERE etat_d_avancement = 100 AND en_cours = 0")->fetchColumn();

?>

<h6 style="color:grey; margin:10px;">TERMINÉE :
...
$stmt = $pdo->query("SELECT id, nom, priorite, due_date, etat_d_avancement FROM todo");
...
<div class="flex-grow-1 p-2 opacity-50">
    <div class="card" data-bs-toggle="modal" data-bs-target="#maModale" data-id="<?= $task['id'] ?>" style="cursor:pointer;">
        <p class="card-text text-decoration-line-through" style="margin-left:15px; margin-top:15px;"><?php echo $task['nom']; ?></p>
