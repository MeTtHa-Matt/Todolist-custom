...
$count = $pdo->query("SELECT COUNT(priorite) FROM todo WHERE etat_d_avancement != 100 AND priorite='urgent'")->fetchColumn();
...
$stmt = $pdo->query("SELECT id, nom, commentaire, priorite, due_date, etat_d_avancement FROM todo");
...
foreach ($tasks as $task):
    ...
    if ($task['etat_d_avancement']!==100 && $task['priorite']==='urgent') {
?>
...
<div class="flex-grow-1 p-2">
    <div class="card" data-bs-toggle="modal" data-bs-target="#maModale" data-id="<?= $task['id'] ?>" style="cursor:pointer;">
        ...
        <div class="d-flex align-items-center gap-2" style="margin-left:15px; margin-bottom:10px;">
            <span style="background-color:#fff0f0; color:#dc3545; font-weight:500; padding: 4px 10px; border-radius: 20px; font-size: 0.85rem; flex-shrink:0;"><?= $task['priorite'] ?></span>
